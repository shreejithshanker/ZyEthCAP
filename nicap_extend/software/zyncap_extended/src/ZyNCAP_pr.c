...
#define HwTimerBaseAddr XPAR_AXI_BRAM_CTRL_3_S_AXI_BASEADDR + 0x00007028
...
int main()
{
	...
	Status = Prefetch_PR_Bitstream("down.bin");
	...
	Status = Config_PR_Bitstream("down.bin",1);
	...
	*/
	...
	u32 delay3,delay4;
	XScuTimer_LoadTimer(TimerInstancePtr, 0xFFFFFFFF);
	delay3 = XScuTimer_GetCounterValue(TimerInstancePtr);
	RecvCount = 0;
	xil_printf("Start Bit over Network test? ");
		while (RecvCount < (sizeof("Yes") - 1)) {
					/* Transmit the data */
					RecvCount += XUartPs_Recv(&Uart_PS,
								   &recvbuffer[RecvCount], sizeof("Yes")-RecvCount);
	...
	xil_printf("Performing initialisation for bitstream over Ethernet Test");

	XScuTimer_Start(TimerInstancePtr);
//	Status = enet_reconfig_test (&ps7_ethernet_0, &Uart_PS,"up.bin");
	Status = enet_reconfig_rxsetup(&ps7_ethernet_0);
	delay4 = delay3 - XScuTimer_GetCounterValue(TimerInstancePtr);
	if (Status == XST_FAILURE)
	    {
	        xil_printf ("\r\n Enet bitstream Test Failed");
	        return 0;
	    }

	xil_printf("\r\n Init Done.. - run Python script for test!!!");

	xil_printf("\r\nRead timer values for bitstream test? ");
	RecvCount = 0;
			while (RecvCount < (sizeof("Yes") - 1)) {
						/* Transmit the data */
						RecvCount += XUartPs_Recv(&Uart_PS,
	...
	u32 timerval;
	u32 timeval, timered;
	timerval = Xil_In32(HwTimerBaseAddr);
	xil_printf("\r\n Delay-reg lower 32 is %d",timerval);
	timeval = timerval / 100000;
	timered = timerval % 100000;
	timerval = Xil_In32(HwTimerBaseAddr+4);
	xil_printf("\r\n Delay-reg mid 32 is %d",timerval);
	//timeval = timeval + (timerval << 32);
	timerval = Xil_In32(HwTimerBaseAddr+8);
	xil_printf("\r\n Delay-reg upper 32 is %d",timerval);
	// timeval = timeval + (timerval << 64);
	//timeval = timeval * 10 / pow(10,6);
	xil_printf("\r\n Delay in PR is %d.%d ms",timeval,timered);
	return 0;
