...
static void EnIntrHandler(void *Calback);
...
static int SetupIntrSystem(XScuGic * IntcInstancePtr, XAxiDma * AxiDmaPtr, u16 TxIntrId, u16 EnIntrId)
{
	...
	Status = XScuGic_Connect(IntcInstancePtr, EnIntrId,(Xil_InterruptHandler)EnIntrHandler,IntcInstancePtr);
	if (Status != XST_SUCCESS) {
		return Status;
	}
...
int Init_Zycap(XScuGic * InterruptController, XEmacPs *EmacPsInstancePtr)
{
  ...
  //print("DMA init done\n\r");
  Status = SetupIntrSystem(InterruptController, &xcdma, TX_INTR_ID, ENET_INTR_ID);
  	if (Status != XST_SUCCESS) {
  ...
  // Set Up ENet
  Status = EmacSetup(InterruptController, EmacPsInstancePtr, \
...
int Net_Bitstream()
{
    ...
    {
    	pres_last = find_last_bs(bs_list);
    	file_size = SD_TransferPartial(modeName, bs_list[pres_last].addr);
        if(file_size == XST_FAILURE)
        {
        	print("bitstream transfer failed\n\r");
        	return XST_FAILURE;
        ...
        pres_first = find_first_bs(bs_list);
        bs_list[pres_last].prev->next = NULL;   //make the second last element as the last element
        strcpy(bs_list[pres_last].name,modeName);
        bs_list[pres_last].size = file_size;
        bs_list[pres_last].prev = NULL;
        bs_list[pres_last].next = &bs_list[pres_first];
        bs_list[pres_first].prev = &bs_list[pres_last];
...
bs_info * getbitDet (char *bs_name)
{
	...
	Status = Prefetch_PR_Bitstream (bs_name);
...
void print_addr()
{
    ...
    {
        current_bs = &bs_list[i];
        printf("BS Num : %d BS Name : %s  BS Addr %s \n",i,current_bs->name,current_bs->addr);
    }
}
